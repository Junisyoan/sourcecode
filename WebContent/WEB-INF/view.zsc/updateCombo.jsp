...
function show(){
	...
	for(var i = current*row;i < Math.min((current+1)*row,projectlList.length);i++){
...
$(document).ready(function(){
	...
	<c:forEach items="${combo.projects}" var = "p" >
		currentList.push("${p.project_id}");
	</c:forEach>
});
...
$(document).ready(function(){
	...
	for(var i = 0;i < Math.min(row,projectList.length);i++){
		
		var td1=$("<td></td>");
		var td2=$("<td></td>").text(projectList[i].name);
		
		var checkbox=$("<input type='checkbox' value='"+projectList[i].id+"'/>");
		if(currentList.indexOf(projectList[i].id) >= 0){
			checkbox.attr("checked",'true');
		}
