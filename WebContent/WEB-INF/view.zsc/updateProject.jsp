...
$(document).ready(function(){
	...
	<c:forEach items="${project.details}" var = "d" >
		currentList.push("${d.detail_id}");
	</c:forEach>
});
...
$(document).ready(function(){
	...
	for(var i = 0;i < Math.min(row,detailList.length);i++){
		
		var td1=$("<td></td>");
		var td2=$("<td></td>").text(detailList[i].name);
		
		var checkbox=$("<input type='checkbox' value='"+detailList[i].id+"'/>");
		if(currentList.indexOf(detailList[i].id) >= 0){
			checkbox.attr("checked",'true');
		}
